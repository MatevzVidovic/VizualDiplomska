...
class Main(DotDict):
	...
	def __call__(self):
		...
		pruning_df.ffill(axis=1, inplace=True)


		for df in (base_df, pruning_df, validation_df):
			...
			df.index = pd.MultiIndex.from_frame(df.index.to_frame().ffill())  # Fill the NaN values from merged cells in the index
			...
			with (self._latex/f'{dataset}.tex').open('w', encoding='utf-8') as latex:
				...
				for model in pruning_df.index.levels[1]:
					...
					for flops in pruning_df.index.levels[2]:
						...
						with ScatterLine(figname, self._figures, xlabel="α", xticklabels=SCATTERLINE_TICKS, ylabel=ylabel) as scline:
						...
						with Bar(figname, self._figures, groups=methods, n=2, ylabel=ylabel) as bar:
							...
							scline.plot([base] * len(SCATTERLINE_TICKS), label="Unpruned", color='black')
